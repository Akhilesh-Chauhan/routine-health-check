...
Runs the master sweep in non-interactive mode (no OTP pop-ups), regenerates
dashboard.html, then emails ONLY when a check or URL is DOWN — or once when
the last DOWN clears. SLOW and DEGRADED states show on the dashboard but
never trigger mail. Intended for a few-times-a-day cron.

Cron usage:
    0 10,14,18 * * * HC_NONINTERACTIVE=1 /usr/bin/env hc monitor sweep >> /path/to/scheduled_sweep.log 2>&1
"""
import json
import os
...
from health_check.orchestration import master as _master
from health_check.orchestration.alerts import send_email

REPORT = str(paths.MASTER_REPORT)
STATE = str(paths.SCHEDULED_STATE)
IST = timezone(timedelta(hours=5, minutes=30))

LOGIN_CMD = {"prod": "hc login prod", "dev": "hc login dev",
             "umang": "hc login umang"}


def _run_phase(name, fn, *args, **kwargs):
    """Call an orchestration step in-process. Logs but never re-raises so a
    dashboard failure doesn't suppress the email path below."""
    try:
        fn(*args, **kwargs)
        return True
    except Exception as e:
        print(f"[sweep] {name} failed: {type(e).__name__}: {e}", flush=True)
        return False
...
def _send_sweep_alert(now, rep, down_scripts):
    ...
    if expired:
        items = "".join(
            f"<li><b>{t}</b> &mdash; <code>DISPLAY=:0 "
            f"{LOGIN_CMD.get(t, '?')}</code></li>" for t in expired)
    ...
    parts.append(f"<p>Dashboard: <code>file://{paths.DASHBOARD_HTML}</code></p>")
...
def main():
    ...
    os.environ.setdefault("HC_NONINTERACTIVE", "1")
    _run_phase("master sweep", _master.main)
    _run_phase("dashboard regen", _dashboard.main, [])
