...
snap = make_snap(ART_DIR, "aistore_", full_page=False)
...
def run():
    ...
    try:
        ...
        try:
            ...
            page.wait_for(CARD_SELECTOR, timeout_ms=10_000)
            body = page.text()
            card_count = page.count(CARD_SELECTOR)
            low = body.lower()
        ...
        try:
            url_before = page.url
            first_card_text = page.inner_text(CARD_SELECTOR)[:80].replace("\n", " ").strip()
            # Click & wait for either navigation OR modal/drawer
            page.click_nav(CARD_SELECTOR, timeout_ms=8_000)
